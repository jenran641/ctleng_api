...
exports.list = (req, res, next) => {
  ...
  if (req.params.selFields == "short") selectFields = ["id,title,type"];
...
exports.getByUrl = (req, res, next) => {
  let url = JSON.parse(req.query.cond);
  ServiceCategory.findByUrl(url, (err, result) => {
    console.log(result)
    if(err) {
      res.send({
        success: 'false'
